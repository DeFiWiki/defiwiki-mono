...
const secp256k1$1 = new ec('secp256k1');
/**
...
async function createJWT(payload, {
  ...
  const fullPayload = _extends({}, timestamps, payload, {
    iss: issuer
  });
...
async function verifyJWT(jwt, options = {
  ...
  const {
    ...
    authenticators,
    issuer
  } = await resolveAuthenticator(options.resolver, header.alg, payload.iss || '', proofPurpose);
  const signer = await verifyJWSDecoded({
    header,
    data,
    signature
  }, authenticators);
...
async function resolveAuthenticator(resolver, alg, issuer, proofPurpose) {
  ...
  if (typeof proofPurpose === 'string') {
    ...
    if (proofPurpose.startsWith('assertion') && !Object.getOwnPropertyNames((_didResult3 = didResult) == null ? void 0 : _didResult3.didDocument).includes('assertionMethod')) {
      didResult.didDocument = _extends({}, didResult.didDocument);
...
async function createJWE(cleartext, encrypters, protectedHeader = {}, aad) {
  if (encrypters[0].alg === 'dir') {
    ...
    for (const encrypter of encrypters) {
      if (!cek) {
        ...
        const recipient = await (encrypter.encryptCek == null ? void 0 : encrypter.encryptCek(cek));
...
async function decryptJWE(jwe, decrypter) {
  validateJWE(jwe);
...
function xc20pDirEncrypter(key) {
  ...
  async function encrypt(cleartext, protectedHeader = {}, aad) {
    ...
    return _extends({}, xc20pEncrypt(cleartext, encodedAad), {
...
function x25519Encrypter(publicKey, kid) {
  ...
  async function encrypt(cleartext, protectedHeader = {}, aad) {
    ...
    return _extends({}, await xc20pDirEncrypter(cek).encrypt(cleartext, protectedHeader, aad), {
...
function x25519Decrypter(receiverSecret) {
  ...
  async function decrypt(sealed, iv, aad, recipient) {
    ...
    recipient = recipient;
    if (((_recipient$header$epk = recipient.header.epk) == null ? void 0 : _recipient$header$epk.crv) !== crv || typeof recipient.header.epk.x == 'undefined') return null;
...
function xc20pAuthDecrypterEcdh1PuV3x25519WithXc20PkwV2(recipientSecret, senderPublicKey) {
  ...
  async function decrypt(sealed, iv, aad, recipient) {
    ...
    if (((_recipient$header$epk2 = recipient.header.epk) == null ? void 0 : _recipient$header$epk2.crv) !== crv || typeof recipient.header.epk.x == 'undefined') return null; // ECDH-1PU requires additional shared secret between
    // static key of sender and static key of recipient
